"use client";

import { usePathname } from "next/navigation";
import Link from "next/link";
import { useEffect, useState } from "react";
import Image from "next/image";

export default function Header() {
  const path = usePathname();

  const [isNotAtTop, setIsNotAtTop] = useState(false);

  useEffect(() => {
    const handleScroll = () => {
      setIsNotAtTop(window.scrollY > 0);
    };
    window.addEventListener("scroll", handleScroll);
    return () => window.removeEventListener("scroll", handleScroll);
  }, []);

  return (
    <div className="h-14 md:h-20 flex justify-center items-end bg-opacity-35">
      <div className="absolute inset-0 backdrop-blur-sm rounded-xl h-1/2" />

      <div
        className={`h-12 md:h-16 max-w-7xl w-full mr-4 ml-4 border transition-all ease-in-out rounded-lg !backdrop-blur-xl flex items-center pl-5 pr-5
        ${
          isNotAtTop
            ? "!border-[var(--header-border-color)] shadow-md"
            : "!border-transparent"
        }
        `}
      >
        <Image
          src="/logo.svg"
          alt="Erick Suehiro Logo"
          width={30}
          height={30}
          className={`filter invert-[var(--filterInvert)] transition-all ease-in-out scale-75 md:scale-100 ${
            isNotAtTop && "rotate-180"
          }`}
        />

        <div className="w-full flex justify-end gap-5">
          {[
            { name: "Home", href: "/" },
            { name: "Projects", href: "/about" },
            { name: "Contact", href: "/about" },
          ].map((item, index) => (
            <Link
              key={index}
              className={`transition-all ease-in-out hover:opacity-100 p-3 ${
                item.href === path ? "!opacity-100" : ""
              } active:scale-95 cursor-pointer opacity-50`}
              href={item.href}
            >
              {item.name}
            </Link>
          ))}
        </div>
      </div>
    </div>

    // <div className="h-[calc((100vh/14))] flex items-start flex-col">
    //   <div className="flex transition-all ease-in-out h-full justify-around items-center text-md md:text-xl z-10 backdrop-blur-xl w-full">
    //     <div className="max-w-7xl w-full p-5 grid grid-cols-[20%80%] items-center">
    // <Image
    //   src="/logo.svg"
    //   alt="Erick Suehiro Logo"
    //   width={30}
    //   height={30}
    //   className={`filter invert-[var(--filterInvert)] transition-all ease-in-out ${
    //     isNotAtTop
    //       ? "rotate-180 scale-75 md:scale-100"
    //       : ""
    //   }`}
    // />

    // <div className="w-full flex justify-end gap-5">
    //   {[
    //     { name: "Home", href: "/" },
    //     { name: "Projects", href: "/about" },
    //     { name: "Contact", href: "/about" },
    //   ].map((item, index) => (
    //     <Link
    //       key={index}
    //       className={`transition-all ease-in-out hover:opacity-100 p-3 ${
    //         item.href === path ? "!opacity-100" : ""
    //       } active:scale-95 cursor-pointer opacity-50`}
    //       href={item.href}
    //     >
    //       {item.name}
    //     </Link>
    //   ))}
    // </div>
    //     </div>
    //   </div>
    //   <hr
    //     className={`border-t ${
    //       isNotAtTop
    //         ? "!border-[var(--header-border-color)]"
    //         : "!border-transparent"
    //     } w-full transition-all ease-in-out !max-w-[100vw]`}
    //   />
    // </div>
  );
}
